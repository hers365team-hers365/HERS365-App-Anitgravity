...
export function signAccessToken(payload: Omit<TokenPayload, 'sessionId' | 'mfaVerified'>): string {
...
export async function validateRefreshToken(token: string): Promise<RefreshTokenData | null> {
  ...
  return {
    ...
    userType: refreshToken[0].userType as UserType,
...
export async function revokeRefreshToken(tokenId: number, reason: string = 'user_logout'): Promise<void> {
  await db
    .update(schema.refreshTokens)
    .set({
      isRevoked: true,
...
export async function revokeAllUserRefreshTokens(userId: number, userType: UserType, reason: string = 'security_action'): Promise<void> {
  await db
    .update(schema.refreshTokens)
    .set({
      isRevoked: true,
    ...
    .where(
      and(
        ...
        eq(schema.refreshTokens.userType, userType),
        eq(schema.refreshTokens.isRevoked, false)
      )
    );
}
...
export async function rotateRefreshToken(
  ...
  ipAddress?: string,
  userAgent?: string
): Promise<{ accessToken: string; refreshToken: string; user: TokenPayload } | null> {
  ...
  const newRefreshTokenId = await storeRefreshToken(
    refreshData.userId,
...
async function getUserDataById(userId: number, userType: UserType): Promise<{ userId: number; email: string; role: UserRole; name: string } | null> {
  switch (userType) {
    case 'athlete':
 ...
 */
export async function generateMFASetup(userId: number, userType: UserType, email: string): Promise<MFASetupData> {
 ...
 */
export async function verifyMFAToken(userId: number, userType: UserType, token: string): Promise<boolean> {
  const mfaData = await db
    .select()
    .from(schema.mfaSecrets)
    .where(
      and(
        eq(schema.mfaSecrets.userId, userId),
        eq(schema.mfaSecrets.userType, userType),
        eq(schema.mfaSecrets.isEnabled, false)
      )
    )
    .limit(1);

  if (mfaData.length === 0) return false;
 ...
 */
export async function verifyMFALogin(userId: number, userType: UserType, token: string): Promise<boolean> {
  const mfaData = await db
    .select()
    .from(schema.mfaSecrets)
    .where(
      and(
        eq(schema.mfaSecrets.userId, userId),
        eq(schema.mfaSecrets.userType, userType),
        eq(schema.mfaSecrets.isEnabled, true)
      )
    )
    .limit(1);
  ...
  for (const hashedCode of hashedCodes) {
    if (await bcrypt.compare(token, hashedCode)) {
      ...
      const updatedCodes = hashedCodes.filter((code: string) => code !== hashedCode);
      await db
        .update(schema.mfaSecrets)
 ...
 */
export async function isMFAEnabled(userId: number, userType: UserType): Promise<boolean> {
  const mfaData = await db
    .select()
    .from(schema.mfaSecrets)
    .where(
 ...
 */
export async function disableMFA(userId: number, userType: UserType): Promise<void> {
  await db
 ...
 */
export async function regenerateBackupCodes(userId: number, userType: UserType): Promise<string[]> {
  const mfaData = await db
    .select()
    .from(schema.mfaSecrets)
    .where(
  ...
  if (mfaData.length === 0) throw new Error('MFA not enabled');
 ...
 */
export async function getUserSessions(userId: number, userType: UserType): Promise<SessionData[]> {
 ...
 */
export async function revokeSession(sessionId: string, reason: string = 'user_action'): Promise<void> {
 ...
 */
export async function revokeAllUserSessions(userId: number, userType: UserType, reason: string = 'security_action'): Promise<void> {
...
export async function getSessionById(sessionId: string): Promise<SessionData | null> {
  ...
  return {
    ...
    userType: session[0].userType as UserType,
...
export async function recordFailedAttempt(
  ...
  ipAddress: string,
  userAgent?: string,
  failureReason: string = 'invalid_credentials'
): Promise<void> {
...
export async function checkAndApplyLockout(email: string, userType: UserType, ipAddress: string): Promise<boolean> {
  ...
  const emailAttempts = await db
    ...
    .where(
      and(
        eq(schema.failedLoginAttempts.email, email),
        eq(schema.failedLoginAttempts.userType, userType),
...
export async function lockAccount(
  ...
  lockDurationMs: number = 15 * 60 * 1000 // 15 minutes default
): Promise<void> {
  // Find user ID
 ...
 */
export async function isAccountLocked(email: string, userType: UserType): Promise<AccountLockData | null> {
  const lockouts = await db
    .select()
    .from(schema.accountLockouts)
    .where(
      and(
        eq(schema.accountLockouts.email, email),
        eq(schema.accountLockouts.userType, userType)
      )
    )
 ...
 * Unlock account
 */
export async function unlockAccount(email: string, userType: UserType, unlockedBy: string = 'admin'): Promise<void> {
  await db
 ...
 */
export function calculateProgressiveDelay(failureCount: number): number {
  if (failureCount <= 3) return 0; // No delay for first 3 attempts
  if (failureCount <= 5) return 1000; // 1 second delay
  if (failureCount <= 7) return 5000; // 5 second delay
  if (failureCount <= 9) return 15000; // 15 second delay
 ...
 */
export async function getRecentFailedAttempts(email: string, userType: UserType, ipAddress: string): Promise<{
...
export async function detectSuspiciousActivity(
  ...
  const recentFailures = await db
    ...
    .where(
      and(
        eq(schema.failedLoginAttempts.email, email),
        gt(schema.failedLoginAttempts.attemptedAt, shortWindow)
      )
    );
...
export async function cleanupSecurityData(): Promise<void> {
  ...
  await db
    ...
    .where(
      and(
        eq(schema.securityAlerts.resolved, true),
        lt(schema.securityAlerts.createdAt, thirtyDaysAgo)
      )
    );
}
 ...
 */
export async function cleanupTokenLifecycle(): Promise<{
  ...
  await db
    ...
    .where(
      and(
        eq(schema.userSessions.isActive, false),
        lt(schema.userSessions.lastActivityAt, thirtyDaysAgo)
      )
    );
 ...
 */
export async function getSecurityMetrics(timeRangeHours: number = 24): Promise<{
  totalLogins: number;
  failedLogins: number;
  activeUsers: number;
  ...
  const [
    ...
    db
      ...
      .where(
        and(
          eq(schema.securityAuditLogs.action, 'login'),
          eq(schema.securityAuditLogs.success, true),
          gt(schema.securityAuditLogs.createdAt, timeRange)
        )
      ),
    db
