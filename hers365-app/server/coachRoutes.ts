...
const coachBoards: Record<number, any[]> = {};
const coachMessages: any[] = [];
...
router.get('/players/search', async (req, res) => {
  const { q, position, state, gradYear, minBreakoutScore, limit = '25', offset = '0' } = req.query as Record<string, string>;
  ...
  // Filter
  if (q) results = results.filter(p => p.name.toLowerCase().includes(q.toLowerCase()) || p.school.toLowerCase().includes(q.toLowerCase()));
...
router.post('/players/:id/save', (req, res) => {
  ...
  if (!coachBoards[coachId]) coachBoards[coachId] = [];

  const existing = coachBoards[coachId].find((p: any) => p.playerId === playerId);
  if (!existing) {
...
router.delete('/players/:id/save', (req, res) => {
  ...
  if (coachBoards[coachId]) {
    coachBoards[coachId] = coachBoards[coachId].filter((p: any) => p.playerId !== playerId);
  }
