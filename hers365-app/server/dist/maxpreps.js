 ...
 */
export async function fetchFlagFootballLeaders(statCategory = 'receiving', state, season = '2025') {
    ...
    console.log('[MaxPreps] API unavailable, returning empty');
    return [];
}
/**
 * Fetch a team's roster and season stats by MaxPreps school GID.
 */
export async function fetchTeamRoster(schoolGID, season = '2025') {
 ...
 */
export async function fetchStateTeamRankings(state, season = '2025') {
 ...
 */
export async function searchMaxPrepsPlayer(name, state) {
    const stateParam = state ? `&state=${state.toUpperCase()}` : '';
 ...
 */
export async function fetchPlayerStats(maxprepsId) {
...
// ─── Normalization  ───────────────────────────────────────────────────────────
function normalizeMaxPrepsPlayer(raw) {
    return {
        source: 'maxpreps',
...
function normalizeMaxPrepsTeam(raw) {
    return {
        source: 'maxpreps',
...
// ─── Mock / Fallback Data ─────────────────────────────────────────────────────
function getMockLeaders(category) {
    const BASE_PLAYER = { source: 'maxpreps', schoolName: 'Demo HS', schoolState: 'TX', position: 'WR', gradYear: 2026 };
...
function getMockTeamRankings(state) {
    return [
...
//# sourceMappingURL=maxpreps.js.map