...
export class ComplianceOrchestrator {
    ...
    setupConfiguration() {
        ...
        this.blobService = BlobServiceClient.fromConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING);
    }
    initializeComponents() {
        // Initialize core services
        this.complianceService = new ComplianceService(this.cosmosClient, this.serviceBusClient);
        this.exportPipeline = new DataExportPipeline(this.cosmosClient, this.blobService, this.serviceBusClient);
        this.deletionPipeline = new DataDeletionPipeline(this.cosmosClient, this.serviceBusClient);
        this.complianceMonitor = new ComplianceMonitor(this.cosmosClient, this.serviceBusClient);
    ...
    async setupDataExportEventHandler() {
        // Subscribe to data export requested events
        const { serviceBusClient } = await import('./service-bus');
        const receiver = await serviceBusClient.subscribeToTopic('user-events', `compliance-export-${Date.now()}`, async (message) => {
            try {
    ...
    async setupDataDeletionEventHandler() {
        // Subscribe to data deletion requested events
        const { serviceBusClient } = await import('./service-bus');
        const receiver = await serviceBusClient.subscribeToTopic('user-events', `compliance-deletion-${Date.now()}`, async (message) => {
            try {
                const event = message.body;
    ...
    async setupComplianceViolationHandler() {
        // Subscribe to compliance violation events
        const { serviceBusClient } = await import('./service-bus');
        const receiver = await serviceBusClient.subscribeToTopic('audit-events', `compliance-alerting-${Date.now()}`, async (message) => {
            try {
                const event = message.body;
     ...
     */
    async generateComplianceReport(reportType = 'daily') {
...
export class ComplianceDashboard {
    ...
    async getDashboardData() {
        const [orchestratorStatus, complianceStatus, dataProcessingStats] = await Promise.all([
...
export class ComplianceUtils {
     ...
     */
    static requiresParentalConsent(userType, age) {
     ...
     */
    static generateComplianceSummary(metrics) {
        const summary = {
...
// ─── EXPORT ORCHESTRATOR ─────────────────────────────────────────────────────
//# sourceMappingURL=compliance-orchestrator.js.map