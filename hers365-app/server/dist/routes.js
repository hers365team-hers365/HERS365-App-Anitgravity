...
router.get('/player-subscription/:playerId', async (req, res) => {
    try {
...
router.post('/player-subscription', async (req, res) => {
    try {
...
router.get('/players/:id/highlights', async (req, res) => {
    const pId = parseInt(req.params.id);
    ...
    const safeHighlights = highlights.map((h, i) => {
        if (i >= 2)
            return { ...h, videoUrl: null, thumbnailUrl: null, locked: true };
        return { ...h, locked: false };
    });
    res.json(safeHighlights);
});
router.post('/players/:id/highlights', async (req, res) => {
    try {
        const pId = parseInt(req.params.id);
...
// AI BOTS & TRAINING
// ----------------------
router.get('/bot/:playerId', async (req, res) => {
    const pId = parseInt(req.params.playerId);
...
router.get('/maxpreps/player', async (req, res) => {
    const { name, school, state } = req.query;
    if (!name)
    ...
    try {
        ...
        if (filtered.length === 1 && filtered[0].maxprepsId) {
            ...
            if (detailedStats)
                filtered[0].stats = { ...filtered[0].stats, ...detailedStats };
        }
...
router.get('/maxpreps/stats/:maxprepsId', async (req, res) => {
    try {
        const stats = await mp.fetchPlayerStats(req.params.maxprepsId);
        if (!stats)
            return res.status(404).json({ error: 'Player not found on MaxPreps' });
...
router.get('/maxpreps/leaders', async (req, res) => {
    const { category = 'receiving', state, season = '2025' } = req.query;
    try {
        const leaders = await mp.fetchFlagFootballLeaders(category, state, season);
...
router.get('/maxpreps/rankings', async (req, res) => {
    const { state = 'TX', season = '2025' } = req.query;
    try {
...
router.get('/maxpreps/team/:schoolGID/roster', async (req, res) => {
