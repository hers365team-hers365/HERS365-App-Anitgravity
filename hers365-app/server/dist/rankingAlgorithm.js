...
export function normalizeValue(value, min, max, lowerIsBetter = false) {
    if (max === min)
        return 50;
...
export function calculateCombineScore(stats) {
    if (!stats)
        return 0;
    let score = 0;
    let weight = 0;
...
export function calculateMaxPrepsScore(stats) {
...
export function calculateZybekScore(stats) {
    if (!stats)
        return 0;
    let score = 0;
    let weight = 0;
...
export function calculateUSATalentIdScore(stats) {
    if (!stats)
        return 0;
    let score = 0;
    let weight = 0;
...
export function getRankingTier(score) {
    if (score >= 90)
        return 'Elite';
    if (score >= 75)
        return 'High Major';
...
//# sourceMappingURL=rankingAlgorithm.js.map