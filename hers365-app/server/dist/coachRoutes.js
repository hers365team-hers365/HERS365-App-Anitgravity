...
router.get('/players/search', async (req, res) => {
    try {
        const { q, position, state, gradYear, minBreakoutScore, maxBreakoutScore, minGpa, maxGpa, minHeight, maxHeight, minWeight, maxWeight, verified, archetype, limit = '25', offset = '0' } = req.query;
        ...
        if (maxBreakoutScore)
            results = results.filter(p => p.breakoutScore <= parseInt(maxBreakoutScore));
        if (minGpa)
            results = results.filter(p => p.gpa >= parseFloat(minGpa));
        if (maxGpa)
            results = results.filter(p => p.gpa <= parseFloat(maxGpa));
        if (archetype)
            results = results.filter(p => p.archetype === archetype);
        if (verified === 'true')
        ...
        if (minWeight)
            results = results.filter(p => p.weight >= parseInt(minWeight));
        if (maxWeight)
            results = results.filter(p => p.weight <= parseInt(maxWeight));
...
router.post('/players/:id/save', async (req, res) => {
    try {
        ...
        const existing = await db.select()
            .from(schema.coachProspects)
            .where(and(eq(schema.coachProspects.coachId, coachId), eq(schema.coachProspects.athleteId, playerId)));
...
router.get('/analytics', async (req, res) => {
    try {
        const coachId = req.user.userId;
        // Get board count
        const boardCount = await db.select({ count: sql `count(*)` })
            .from(schema.coachProspects)
            .where(eq(schema.coachProspects.coachId, coachId));
        // Get messages sent count
        const messagesSent = await db.select({ count: sql `count(*)` })
            .from(schema.messages)
            .where(eq(schema.messages.coachId, coachId));
